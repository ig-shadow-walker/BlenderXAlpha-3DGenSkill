...
compatibility: >
  ...
  instance. Spends real Alpha3D credits per generated asset — this skill
...
Two files carry the parts of this skill that are pure mechanism rather than
...
assume either is up just because the tools appear in your tool list —
verify with a cheap, free call to each:
...
- **source** — `generate`, `primitive`, or `skip`. This decision matters for
  ...
  it actually earns its keep — hero props, organic shapes, characters,
...
limiting how many jobs you can have in flight — each call is independent —
so submit all of them back-to-back rather than waiting for one to finish
