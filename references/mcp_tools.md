...
| `uv_unwrap` | Generate clean UV maps | — |
| `texture_3d` | Re-texture from a prompt or reference image | `prompt` or `image_url` (one required) |
| `rig_3d` | Auto-rig a humanoid mesh in a T-pose (armature + skin weights) | — |
| `segment_3d` | Split into labeled semantic parts, each its own GLB | — |
